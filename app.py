...
with st.sidebar:
    ...
    if st.button("Calculate Position Size", use_container_width=True):
        result = calculate_position_size(total_capital, entry_price, stop_loss)
        if result.is_valid:
            st.success(result.message)
            col_a, col_b = st.columns(2)
            col_a.metric("Shares to Buy", result.shares)
            col_b.metric("Position Value", f"₹{result.position_value:,.2f}")
            st.metric("Max Risk", f"₹{result.risk_amount:,.2f}")
